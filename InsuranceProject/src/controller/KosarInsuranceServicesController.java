...
import model.service.KosarInsuranceServicesService;
import view.KosarInsuranceServices;

import javax.swing.*;
import java.util.List;

public class KosarInsuranceServicesController {

    private static KosarInsuranceServicesController kosarInsuranceServicesController = new KosarInsuranceServicesController();

    public static KosarInsuranceServicesController getInstance() {
        return kosarInsuranceServicesController;
    }

    private KosarInsuranceServicesController() {
    ...
    public void insert(String fullName, String nationalId, String certificateId, String fatherName, String phoneNumber, String homePhone, String birthDate, String address, String nationality, String userType, String insuranceType, int contractTerm, String registrationDate, long paymentTrackingCode, String documentNumber, long paymentCode, long bill) throws Exception {
        ...
        if(!select3(nationalId, insuranceType)) {
            KosarInsuranceCustomersController.getInstance().insert(fullName, nationalId, certificateId, fatherName, phoneNumber, homePhone, birthDate, address, nationality, userType);
            KosarInsuranceServicesService.getInstance().save(new KosarInsuranceServicesEntity().setNationalId(nationalId).setInsuranceType(insuranceType).setContractTerm(contractTerm).setRegistrationDate(registrationDate).setPaymentTrackingCode(paymentTrackingCode).setDocumentNumber(documentNumber).setPaymentCode(paymentCode).setBill(bill).setServiceType("register"));
            ReportController.getInstance().insert("kosar", insuranceType, "register insurance", contractTerm, registrationDate, paymentCode, bill, nationalId, nationality, userType);
            KosarInsuranceServices.jFrame1.dispose();
        ...
        if(select(paymentCode)) {
            if(select1(paymentCode)) {
                if(select2(paymentCode)) {
                    KosarInsuranceServicesService.getInstance().edit(new KosarInsuranceServicesEntity().setPaymentCode(paymentCode).setPaymentTrackingCode(paymentTrackingCode).setBill(bill).setServiceType("pay"));
                    KosarInsuranceServices.jFrame2.dispose();
    ...
    public void update(long paymentCode) throws Exception {
        ...
        if(select(paymentCode)) {
            if(select1(paymentCode)) {
                if(!select2(paymentCode)) {
                    KosarInsuranceServicesService.getInstance().edit(paymentCode, "cancellation");
                    KosarInsuranceServices.jFrame3.dispose();
    ...
    public boolean select(long paymentCode) throws Exception {
        ...
        for(KosarInsuranceServicesEntity kosarInsuranceServicesEntity : kosarInsuranceServicesList) {
            if(paymentCode == kosarInsuranceServicesEntity.getPaymentCode()) {
                check = true;
                break;
            }
        }
    ...
    public boolean select1(long paymentCode) throws Exception {
        ...
        List<KosarInsuranceServicesEntity> kosarInsuranceServicesList = KosarInsuranceServicesService.getInstance().report();
        for(KosarInsuranceServicesEntity kosarInsuranceServicesEntity : kosarInsuranceServicesList) {
            if(paymentCode == kosarInsuranceServicesEntity.getPaymentCode() && !kosarInsuranceServicesEntity.getServiceType().equals("cancellation")) {
                check = true;
                break;
            }
        }
    ...
    public boolean select2(long paymentCode) throws Exception {
        ...
        List<KosarInsuranceServicesEntity> kosarInsuranceServicesList = KosarInsuranceServicesService.getInstance().report();
        for(KosarInsuranceServicesEntity kosarInsuranceServicesEntity : kosarInsuranceServicesList) {
            if(paymentCode == kosarInsuranceServicesEntity.getPaymentCode() && !kosarInsuranceServicesEntity.getServiceType().equals("cancellation") && kosarInsuranceServicesEntity.getBill() != 0) {
                check = true;
                break;
            }
        }
    ...
    public boolean select3(String nationalId, String insuranceType) throws Exception {
        ...
        List<KosarInsuranceServicesEntity> kosarInsuranceServicesList = KosarInsuranceServicesService.getInstance().report();
        for(KosarInsuranceServicesEntity kosarInsuranceServicesEntity : kosarInsuranceServicesList) {
            if(nationalId.equals(kosarInsuranceServicesEntity.getNationalId()) && insuranceType.equals(kosarInsuranceServicesEntity.getInsuranceType()) && !kosarInsuranceServicesEntity.getServiceType().equals("cancellation")) {
                check = true;
                break;
            }
        }
