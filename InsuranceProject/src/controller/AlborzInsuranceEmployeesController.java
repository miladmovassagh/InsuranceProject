...
import model.service.AlborzInsuranceEmployeesService;
import view.AlborzInsurance;
import view.AlborzInsuranceEmployeesLogin;
import view.AlborzInsuranceEmployeesSignUp;
import view.AlborzInsuranceServices;

import javax.swing.*;
import java.util.*;

public class AlborzInsuranceEmployeesController {

    private static AlborzInsuranceEmployeesController alborzInsuranceEmployeesController = new AlborzInsuranceEmployeesController();

    public static AlborzInsuranceEmployeesController getInstance() {
        return alborzInsuranceEmployeesController;
    }

    private AlborzInsuranceEmployeesController() {
    ...
    public void select(String username, String password) throws Exception {
        ...
        boolean check = false;
        List<AlborzInsuranceEmployeesEntity> alborzInsuranceEmployeesList = AlborzInsuranceEmployeesService.getInstance().report();
        for(AlborzInsuranceEmployeesEntity alborzInsuranceEmployeesEntity : alborzInsuranceEmployeesList)
            if (username.equals(alborzInsuranceEmployeesEntity.getUsername()) && password.equals(alborzInsuranceEmployeesEntity.getPassword())) {
                check = true;
                new AlborzInsuranceServices();
                AlborzInsuranceEmployeesLogin.jFrame.dispose();
            }
        if(!check)
    ...
    public boolean select(String nationalId) throws Exception {
        ...
        List<AlborzInsuranceEmployeesEntity> alborzInsuranceEmployeesList = AlborzInsuranceEmployeesService.getInstance().report();
        for(AlborzInsuranceEmployeesEntity alborzInsuranceEmployeesEntity : alborzInsuranceEmployeesList) {
            if (nationalId.equals(alborzInsuranceEmployeesEntity.getNationalId())) {
                check = true;
                break;
            }
        }
