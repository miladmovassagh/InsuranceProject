...
import model.service.PasargadInsuranceEmployeesService;
import view.PasargadInsurance;
import view.PasargadInsuranceEmployeesLogin;
import view.PasargadInsuranceEmployeesSignUp;
import view.PasargadInsuranceServices;

import javax.swing.*;
import java.util.*;

public class PasargadInsuranceEmployeesController {

    private static PasargadInsuranceEmployeesController pasargadInsuranceEmployeesController = new PasargadInsuranceEmployeesController();

    public static PasargadInsuranceEmployeesController getInstance() {
        return pasargadInsuranceEmployeesController;
    }

    private PasargadInsuranceEmployeesController() {
    ...
    public void select(String username, String password) throws Exception {
        ...
        boolean check = false;
        List<PasargadInsuranceEmployeesEntity> pasargadInsuranceEmployeesList = PasargadInsuranceEmployeesService.getInstance().report();
        for(PasargadInsuranceEmployeesEntity pasargadInsuranceEmployeesEntity : pasargadInsuranceEmployeesList)
            if (username.equals(pasargadInsuranceEmployeesEntity.getUsername()) && password.equals(pasargadInsuranceEmployeesEntity.getPassword())) {
                check = true;
                new PasargadInsuranceServices();
                PasargadInsuranceEmployeesLogin.jFrame.dispose();
            }
        if(!check)
    ...
    public boolean select(String nationalId) throws Exception {
        ...
        List<PasargadInsuranceEmployeesEntity> pasargadInsuranceEmployeesList = PasargadInsuranceEmployeesService.getInstance().report();
        for(PasargadInsuranceEmployeesEntity pasargadInsuranceEmployeesEntity : pasargadInsuranceEmployeesList) {
            if (nationalId.equals(pasargadInsuranceEmployeesEntity.getNationalId())) {
                check = true;
                break;
            }
        }
