...
import model.service.AlborzInsuranceServicesService;
import view.AlborzInsuranceServices;

import javax.swing.*;
import java.util.List;

public class AlborzInsuranceServicesController {

    private static AlborzInsuranceServicesController alborzInsuranceServicesController = new AlborzInsuranceServicesController();

    public static AlborzInsuranceServicesController getInstance() {
        return alborzInsuranceServicesController;
    }

    private AlborzInsuranceServicesController() {
    ...
    public void insert(String fullName, String nationalId, String certificateId, String fatherName, String phoneNumber, String homePhone, String birthDate, String address, String nationality, String userType, String insuranceType, int contractTerm, String registrationDate, long paymentTrackingCode, String documentNumber, long paymentCode, long bill) throws Exception {
        ...
        if(!select3(nationalId, insuranceType)) {
            AlborzInsuranceCustomersController.getInstance().insert(fullName, nationalId, certificateId, fatherName, phoneNumber, homePhone, birthDate, address, nationality, userType);
            AlborzInsuranceServicesService.getInstance().save(new AlborzInsuranceServicesEntity().setNationalId(nationalId).setInsuranceType(insuranceType).setContractTerm(contractTerm).setRegistrationDate(registrationDate).setPaymentTrackingCode(paymentTrackingCode).setDocumentNumber(documentNumber).setPaymentCode(paymentCode).setBill(bill).setServiceType("register"));
            ReportController.getInstance().insert("alborz", insuranceType, "register insurance", contractTerm, registrationDate, paymentCode, bill, nationalId, nationality, userType);
            AlborzInsuranceServices.jFrame1.dispose();
        ...
        if(select(paymentCode)) {
            if(select1(paymentCode)) {
                if(select2(paymentCode)) {
                    AlborzInsuranceServicesService.getInstance().edit(new AlborzInsuranceServicesEntity().setPaymentCode(paymentCode).setPaymentTrackingCode(paymentTrackingCode).setBill(bill).setServiceType("pay"));
                    AlborzInsuranceServices.jFrame2.dispose();
    ...
    public void update(long paymentCode) throws Exception {
        ...
        if(select(paymentCode)) {
            if(select1(paymentCode)) {
                if(!select2(paymentCode)) {
                    AlborzInsuranceServicesService.getInstance().edit(paymentCode, "cancellation");
                    AlborzInsuranceServices.jFrame3.dispose();
    ...
    public boolean select(long paymentCode) throws Exception {
        ...
        for(AlborzInsuranceServicesEntity alborzInsuranceServicesEntity : alborzInsuranceServicesList) {
            if(paymentCode == alborzInsuranceServicesEntity.getPaymentCode()) {
                check = true;
                break;
            }
        }
    ...
    public boolean select1(long paymentCode) throws Exception {
        ...
        List<AlborzInsuranceServicesEntity> alborzInsuranceServicesList = AlborzInsuranceServicesService.getInstance().report();
        for(AlborzInsuranceServicesEntity alborzInsuranceServicesEntity : alborzInsuranceServicesList) {
            if(paymentCode == alborzInsuranceServicesEntity.getPaymentCode() && !alborzInsuranceServicesEntity.getServiceType().equals("cancellation")) {
                check = true;
                break;
            }
        }
    ...
    public boolean select2(long paymentCode) throws Exception {
        ...
        List<AlborzInsuranceServicesEntity> alborzInsuranceServicesList = AlborzInsuranceServicesService.getInstance().report();
        for(AlborzInsuranceServicesEntity alborzInsuranceServicesEntity : alborzInsuranceServicesList) {
            if(paymentCode == alborzInsuranceServicesEntity.getPaymentCode() && !alborzInsuranceServicesEntity.getServiceType().equals("cancellation") && alborzInsuranceServicesEntity.getBill() != 0) {
                check = true;
                break;
            }
        }
    ...
    public boolean select3(String nationalId, String insuranceType) throws Exception {
        ...
        List<AlborzInsuranceServicesEntity> alborzInsuranceServicesList = AlborzInsuranceServicesService.getInstance().report();
        for(AlborzInsuranceServicesEntity alborzInsuranceServicesEntity : alborzInsuranceServicesList) {
            if(nationalId.equals(alborzInsuranceServicesEntity.getNationalId()) && insuranceType.equals(alborzInsuranceServicesEntity.getInsuranceType()) && !alborzInsuranceServicesEntity.getServiceType().equals("cancellation")) {
                check = true;
                break;
            }
        }
