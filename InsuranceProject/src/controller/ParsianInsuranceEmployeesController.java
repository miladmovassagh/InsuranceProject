...
import model.service.ParsianInsuranceEmployeesService;
import view.ParsianInsurance;
import view.ParsianInsuranceEmployeesLogin;
import view.ParsianInsuranceEmployeesSignUp;
import view.ParsianInsuranceServices;

import javax.swing.*;
import java.util.List;

public class ParsianInsuranceEmployeesController {

    private static ParsianInsuranceEmployeesController parsianInsuranceEmployeesController = new ParsianInsuranceEmployeesController();

    public static ParsianInsuranceEmployeesController getInstance() {
        return parsianInsuranceEmployeesController;
    }

    private ParsianInsuranceEmployeesController() {
    ...
    public void select(String username, String password) throws Exception {
        ...
        boolean check = false;
        List<ParsianInsuranceEmployeesEntity> parsianInsuranceEmployeesList = ParsianInsuranceEmployeesService.getInstance().report();
        for(ParsianInsuranceEmployeesEntity parsianInsuranceEmployeesEntity : parsianInsuranceEmployeesList)
            if (username.equals(parsianInsuranceEmployeesEntity.getUsername()) && password.equals(parsianInsuranceEmployeesEntity.getPassword())) {
                check = true;
                new ParsianInsuranceServices();
                ParsianInsuranceEmployeesLogin.jFrame.dispose();
            }
        if(!check)
    ...
    public boolean select(String nationalId) throws Exception {
        ...
        List<ParsianInsuranceEmployeesEntity> parsianInsuranceEmployeesList = ParsianInsuranceEmployeesService.getInstance().report();
        for(ParsianInsuranceEmployeesEntity parsianInsuranceEmployeesEntity : parsianInsuranceEmployeesList) {
            if (nationalId.equals(parsianInsuranceEmployeesEntity.getNationalId())) {
                check = true;
                break;
            }
        }
