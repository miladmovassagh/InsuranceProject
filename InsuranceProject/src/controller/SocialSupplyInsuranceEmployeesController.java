...
import model.service.SocialSupplyInsuranceEmployeesService;
import view.SocialSupplyInsurance;
import view.SocialSupplyInsuranceEmployeesLogin;
import view.SocialSupplyInsuranceEmployeesSignUp;
import view.SocialSupplyInsuranceServices;

import javax.swing.*;
import java.util.*;

public class SocialSupplyInsuranceEmployeesController {

    private static SocialSupplyInsuranceEmployeesController socialSupplyInsuranceEmployeesController = new SocialSupplyInsuranceEmployeesController();

    public static SocialSupplyInsuranceEmployeesController getInstance() {
        return socialSupplyInsuranceEmployeesController;
    }

    private SocialSupplyInsuranceEmployeesController() {
    ...
    public void select(String username, String password) throws Exception {
        ...
        boolean check = false;
        List<SocialSupplyInsuranceEmployeesEntity> socialSupplyInsuranceEmployeesList = SocialSupplyInsuranceEmployeesService.getInstance().report();
        for(SocialSupplyInsuranceEmployeesEntity socialSupplyInsuranceEmployeesEntity : socialSupplyInsuranceEmployeesList)
            if (username.equals(socialSupplyInsuranceEmployeesEntity.getUsername()) && password.equals(socialSupplyInsuranceEmployeesEntity.getPassword())) {
                check = true;
                new SocialSupplyInsuranceServices();
                SocialSupplyInsuranceEmployeesLogin.jFrame.dispose();
            }
        if(!check)
    ...
    public boolean select(String nationalId) throws Exception {
        ...
        List<SocialSupplyInsuranceEmployeesEntity> socialSupplyInsuranceEmployeesList = SocialSupplyInsuranceEmployeesService.getInstance().report();
        for(SocialSupplyInsuranceEmployeesEntity socialSupplyInsuranceEmployeesEntity : socialSupplyInsuranceEmployeesList) {
            if (nationalId.equals(socialSupplyInsuranceEmployeesEntity.getNationalId())) {
                check = true;
                break;
            }
        }
