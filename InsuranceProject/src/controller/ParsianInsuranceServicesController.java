...
import model.service.ParsianInsuranceServicesService;
import view.ParsianInsuranceServices;

import javax.swing.*;
import java.util.List;

public class ParsianInsuranceServicesController {

    private static ParsianInsuranceServicesController parsianInsuranceServicesController = new ParsianInsuranceServicesController();

    public static ParsianInsuranceServicesController getInstance() {
        return parsianInsuranceServicesController;
    }

    private ParsianInsuranceServicesController() {
    ...
    public void insert(String fullName, String nationalId, String certificateId, String fatherName, String phoneNumber, String homePhone, String birthDate, String address, String nationality, String userType, String insuranceType, int contractTerm, String registrationDate, long paymentTrackingCode, String documentNumber, long paymentCode, long bill) throws Exception {
        ...
        if(!select3(nationalId, insuranceType)) {
            ParsianInsuranceCustomersController.getInstance().insert(fullName, nationalId, certificateId, fatherName, phoneNumber, homePhone, birthDate, address, nationality, userType);
            ParsianInsuranceServicesService.getInstance().save(new ParsianInsuranceServicesEntity().setNationalId(nationalId).setInsuranceType(insuranceType).setContractTerm(contractTerm).setRegistrationDate(registrationDate).setPaymentTrackingCode(paymentTrackingCode).setDocumentNumber(documentNumber).setPaymentCode(paymentCode).setBill(bill).setServiceType("register"));
            ReportController.getInstance().insert("parsian", insuranceType, "register insurance", contractTerm, registrationDate, paymentCode, bill, nationalId, nationality, userType);
            ParsianInsuranceServices.jFrame1.dispose();
        ...
        if(select(paymentCode)) {
            if(select1(paymentCode)) {
                if(select2(paymentCode)) {
                    ParsianInsuranceServicesService.getInstance().edit(new ParsianInsuranceServicesEntity().setPaymentCode(paymentCode).setPaymentTrackingCode(paymentTrackingCode).setBill(bill).setServiceType("pay"));
                    ParsianInsuranceServices.jFrame2.dispose();
    ...
    public void update(long paymentCode) throws Exception {
        ...
        if(select(paymentCode)) {
            if(select1(paymentCode)) {
                if(!select2(paymentCode)) {
                    ParsianInsuranceServicesService.getInstance().edit(paymentCode, "cancellation");
                    ParsianInsuranceServices.jFrame3.dispose();
    ...
    public boolean select(long paymentCode) throws Exception {
        ...
        for(ParsianInsuranceServicesEntity parsianInsuranceServicesEntity : parsianInsuranceServicesList) {
            if(paymentCode == parsianInsuranceServicesEntity.getPaymentCode()) {
                check = true;
                break;
            }
        }
    ...
    public boolean select1(long paymentCode) throws Exception {
        ...
        List<ParsianInsuranceServicesEntity> parsianInsuranceServicesList = ParsianInsuranceServicesService.getInstance().report();
        for(ParsianInsuranceServicesEntity parsianInsuranceServicesEntity : parsianInsuranceServicesList) {
            if(paymentCode == parsianInsuranceServicesEntity.getPaymentCode() && !parsianInsuranceServicesEntity.getServiceType().equals("cancellation")) {
                check = true;
                break;
            }
        }
    ...
    public boolean select2(long paymentCode) throws Exception {
        ...
        List<ParsianInsuranceServicesEntity> parsianInsuranceServicesList = ParsianInsuranceServicesService.getInstance().report();
        for(ParsianInsuranceServicesEntity parsianInsuranceServicesEntity : parsianInsuranceServicesList) {
            if(paymentCode == parsianInsuranceServicesEntity.getPaymentCode() && !parsianInsuranceServicesEntity.getServiceType().equals("cancellation") && parsianInsuranceServicesEntity.getBill() != 0) {
                check = true;
                break;
            }
        }
    ...
    public boolean select3(String nationalId, String insuranceType) throws Exception {
        ...
        List<ParsianInsuranceServicesEntity> parsianInsuranceServicesList = ParsianInsuranceServicesService.getInstance().report();
        for(ParsianInsuranceServicesEntity parsianInsuranceServicesEntity : parsianInsuranceServicesList) {
            if(nationalId.equals(parsianInsuranceServicesEntity.getNationalId()) && insuranceType.equals(parsianInsuranceServicesEntity.getInsuranceType()) && !parsianInsuranceServicesEntity.getServiceType().equals("cancellation")) {
                check = true;
                break;
            }
        }
