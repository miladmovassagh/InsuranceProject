...
import model.service.SocialSupplyInsuranceServicesService;
import view.SocialSupplyInsuranceServices;

import javax.swing.*;
import java.util.List;

public class SocialSupplyInsuranceServicesController {

    private static SocialSupplyInsuranceServicesController socialSupplyInsuranceServicesController = new SocialSupplyInsuranceServicesController();

    public static SocialSupplyInsuranceServicesController getInstance() {
        return socialSupplyInsuranceServicesController;
    }

    private SocialSupplyInsuranceServicesController() {
    ...
    public void insert(String fullName, String nationalId, String certificateId, String fatherName, String phoneNumber, String homePhone, String birthDate, String address, String nationality, String userType, String insuranceType, int contractTerm, String registrationDate, long paymentTrackingCode, String documentNumber, long paymentCode, long bill) throws Exception {
        ...
        if(!select3(nationalId, insuranceType)) {
            SocialSupplyInsuranceCustomersController.getInstance().insert(fullName, nationalId, certificateId, fatherName, phoneNumber, homePhone, birthDate, address, nationality, userType);
            SocialSupplyInsuranceServicesService.getInstance().save(new SocialSupplyInsuranceServicesEntity().setNationalId(nationalId).setInsuranceType(insuranceType).setContractTerm(contractTerm).setRegistrationDate(registrationDate).setPaymentTrackingCode(paymentTrackingCode).setDocumentNumber(documentNumber).setPaymentCode(paymentCode).setBill(bill).setServiceType("register"));
            ReportController.getInstance().insert("social supply", insuranceType, "register insurance", contractTerm, registrationDate, paymentCode, bill, nationalId, nationality, userType);
            SocialSupplyInsuranceServices.jFrame1.dispose();
            ...
            new SocialSupplyInsuranceServices();
        }
        else
        ...
        if(select(paymentCode)) {
            if(select1(paymentCode)) {
                if(select2(paymentCode)) {
                    SocialSupplyInsuranceServicesService.getInstance().edit(new SocialSupplyInsuranceServicesEntity().setPaymentCode(paymentCode).setPaymentTrackingCode(paymentTrackingCode).setBill(bill).setServiceType("pay"));
                    SocialSupplyInsuranceServices.jFrame2.dispose();
    ...
    public void update(long paymentCode) throws Exception {
        ...
        if(select(paymentCode)) {
            if(select1(paymentCode)) {
                if(!select2(paymentCode)) {
                    SocialSupplyInsuranceServicesService.getInstance().edit(paymentCode, "cancellation");
                    SocialSupplyInsuranceServices.jFrame3.dispose();
    ...
    public boolean select(long paymentCode) throws Exception {
        ...
        for(SocialSupplyInsuranceServicesEntity socialSupplyInsuranceServicesEntity : socialSupplyInsuranceServicesList) {
            if(paymentCode == socialSupplyInsuranceServicesEntity.getPaymentCode()) {
                check = true;
                break;
            }
        }
    ...
    public boolean select1(long paymentCode) throws Exception {
        ...
        List<SocialSupplyInsuranceServicesEntity> socialSupplyInsuranceServicesList = SocialSupplyInsuranceServicesService.getInstance().report();
        for(SocialSupplyInsuranceServicesEntity socialSupplyInsuranceServicesEntity : socialSupplyInsuranceServicesList) {
            if(paymentCode == socialSupplyInsuranceServicesEntity.getPaymentCode() && !socialSupplyInsuranceServicesEntity.getServiceType().equals("cancellation")) {
                check = true;
                break;
            }
        }
    ...
    public boolean select2(long paymentCode) throws Exception {
        ...
        List<SocialSupplyInsuranceServicesEntity> socialSupplyInsuranceServicesList = SocialSupplyInsuranceServicesService.getInstance().report();
        for(SocialSupplyInsuranceServicesEntity socialSupplyInsuranceServicesEntity : socialSupplyInsuranceServicesList) {
            if(paymentCode == socialSupplyInsuranceServicesEntity.getPaymentCode() && !socialSupplyInsuranceServicesEntity.getServiceType().equals("cancellation") && socialSupplyInsuranceServicesEntity.getBill() != 0) {
                check = true;
                break;
            }
        }
    ...
    public boolean select3(String nationalId, String insuranceType) throws Exception {
        ...
        List<SocialSupplyInsuranceServicesEntity> socialSupplyInsuranceServicesList = SocialSupplyInsuranceServicesService.getInstance().report();
        for(SocialSupplyInsuranceServicesEntity socialSupplyInsuranceServicesEntity : socialSupplyInsuranceServicesList) {
            if(nationalId.equals(socialSupplyInsuranceServicesEntity.getNationalId()) && insuranceType.equals(socialSupplyInsuranceServicesEntity.getInsuranceType()) && !socialSupplyInsuranceServicesEntity.getServiceType().equals("cancellation")) {
                check = true;
                break;
            }
        }
